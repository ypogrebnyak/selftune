function vm() {
    ...
    self.visibleMessage = ko.observable(false);
    self.spanClicked = function(){
        $.ajax({
            type: 'GET'
            ,
            url: '/BaseProjectWeb/resources/subscribe/subscribe?email='+self.email()
            ,
            success: function(res){
                console.log(res);
                $.pnotify({
                    text: (res.response == undefined) ? res.error : res.response , 
                    hide: true
                });
                self.message((res.response == undefined) ? res.error : res.response);
